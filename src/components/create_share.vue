<template>
    <arco-modal width="70%" v-bind:visible="open" @cancel="emit('close')" :footer="false" unmount-on-close @close="handleAfterClose">
      ...
      <div v-if="!loading && !success">
        ...
        <p style="color: red;">分享类型、有效期、提取码创建后不支持更改，请慎重选择</p>
        <arco-form :model="share" :style="{ width: '600px', marginTop: '30px' }" @submit-success="submit">
            <arco-form-item :rules="[{required:true, message:'系统错误，请刷新页面重试'}]" field="content_id" tooltip="无需填写，文章唯一标识符" label="文章ID">
                <arco-input v-model="share.content_id" readonly disabled/>
            </arco-form-item>
            <arco-form-item :rules="[{required:true, message:'系统错误，请刷新页面重试'}]" field="creator_id" tooltip="无需填写，编辑器账户ID（标识用）" label="创建者ID">
                <arco-input v-model="share.creator_id" readonly disabled/>
            </arco-form-item>
            <arco-form-item field="code" tooltip="可选，如果需要对文章进行加密可以输入提取码" label="提取码">
                <arco-input-password :disabled="creating" v-model="share.code" placeholder="提取码（可选）"/>
            </arco-form-item>
            <arco-form-item :rules="[{required:true, message:'请选择有效期'}]" field="vaild_before" label="有效期" tooltip="设置分享的有效期，超过有效期后分享失效">
                <arco-select :disabled="creating" @change="change" v-model:model-value="share.vaild_before" :style="{width:'200px'}" placeholder="设置有效期">
                    <arco-option :value="-1">不设置有效期</arco-option>
                    <arco-option :value="3600">1 小时</arco-option>
                    <arco-option :value="10800">3 小时</arco-option>
                    <arco-option :value="21600">6 小时</arco-option>
                    <arco-option :value="43200">12 小时</arco-option>
                    <arco-option :value="86400">1 天</arco-option>
                    <arco-option :value="604800">3 天</arco-option>
                    <arco-option :value="1209600">7 天</arco-option>
                    <arco-option :value="2592000">30 天</arco-option>
                </arco-select>
            </arco-form-item>
            <arco-form-item :rules="[{required:true, message:'请选择分享类型'}]" field="share_type" label="分享类型">
                <arco-select :disabled="creating" @change="change" v-model:model-value="share.share_type" :style="{width:'200px'}" placeholder="设置分享类型">
                    <arco-option :value="0">私密分享</arco-option>
                    <arco-option :value="1">公共分享</arco-option>
                </arco-select>
                <template #extra>
                    <div>私密分享：只有被授权的用户可以查看文章<br/>公共分享：互联网上任何获得了分享链接的人都可查看</div>
                </template>
            </arco-form-item>
            ...
            <arco-form-item>
                <arco-button :loading="creating" id="submit" type="primary" size="large" html-type="submit" shape="round"><template #icon><icon-check/></template>创建分享</arco-button>
            </arco-form-item>
        </arco-form>
      </div>
    </arco-modal>
...
const getContentData = () => {
    Axios('/api/v2/content/?id=' + prop['id'])
      .then((r) => {
        if (r['data']['code'] != 0) {
          ...
          share.content_id = r['data']['data']['_id']['$oid']
          share.creator_id = r['data']['data']['author_id']['$oid']
          loading.value = false
        }
      })
      .catch((e) => {
...
const submit = (errors) => {
      creating.value = true
      Axios.post('/api/v2.1/share/create', share)
        .then((r) => {
          if (r['data']['code'] != 0) {
            Message.error(r['data']['msg'])
          }
