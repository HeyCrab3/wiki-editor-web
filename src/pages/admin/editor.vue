<template>
    <a-layout style="height: 100%;">
      <a-layout-header>
        <a-page-header
            ...
            >
            <template #extra>
                <span style="color: #FFF; margin-bottom: 6px; display: inline-block">请按照《审核规范》进行审核，审核完毕后在右上角选择审核状态，不通过则须填写原因。如有恶意审批者，一经发现，永久封禁。</span>
                <a-button @click="passVerify" status="success" shape="round"><icon-check style="margin-right: 5px;"/>审核通过</a-button>
                <a-button @click="returnVerify" status="danger" shape="round"><icon-close style="margin-right: 5px;"/>审核不通过</a-button>
            </template>
            </a-page-header>
      </a-layout-header>
      <a-layout>
  ...
  <a-modal v-model:visible="v2" :on-before-ok="l">
    <template #title>
      请填写驳回原因
    </template>
    <div style="margin-top: 20px;">
      <a-input v-model="reason" placeholder="驳回原因" allow-clear>
        <template #prefix>
          <icon-edit />
        </template>
      </a-input>
...
onMounted(() => {
    ...
    Axios(`/api/v1/content_manage/zone/3/getContent?id=${router.currentRoute.value.params['id']}`)
    .then(function(r){
        if (r['data']['code'] != 0){
...
const passVerify = () => {
    Modal.confirm({
        ...
        onOk: function(){
            Axios({
                url: '/api/v1/content_manage/zone/3/updateValidateStatus',
                method: 'POST',
                data: {
...
const save = () => {
    loading.value = true;
    Axios({
        url: '/api/v1/content_manage/zone/3/updateContent',
        method: 'POST',
        data: {
