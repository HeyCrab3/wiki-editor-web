<template>
    <Modal :open="prop.show" :footer="null" title="上传头像" :confirm-loading="loading" @ok="handleOk" @cancel="emit('close')">
      <p>这是你当前的头像：<Avatar :src="user.user_data.d.avatar" /></p>
      <p>在下面上传你的新头像：</p>
...
const upload = (option: RequestOption) => {
  ...
  .then((r) => {
    ...
    else {Message.success('头像已更新，刷新后生效'); onSuccess(); emit('close')}
  })
  .catch((e) => {
    loading.value = false
    Modal.error({
